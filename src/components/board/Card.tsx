...
export const Card = ({ card, onClick }: CardProps) => {
  ...
  return (
    <div
      ...
      className={`
        bg-white dark:bg-[#18181b] p-3 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-800/80
    ...
    >
      ...
      {(card.description || card.due_date) && (
        <div className="mt-2.5 flex items-center gap-3 text-zinc-400 dark:text-zinc-500">
          ...
          {card.due_date && (
            <div className={`flex items-center gap-1 text-[11px] font-medium ${isOverdue ? 'text-red-500 dark:text-red-400' : ''}`} title="Due date">
