...
export const CardModal = ({ card, onClose, onUpdate, onDelete }: CardModalProps) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Warn on Escape if there are unsaved changes
      if (e.key === 'Escape') {
        if (isDirty) {
  ...
  // Intercept backdrop click if dirty
  ...
  const performDelete = async () => {
    // Track delete in-flight state independently from saving
    setIsDeleting(true);
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/40 dark:bg-black/60 backdrop-blur-sm animate-backdrop-fade"
    ...
    >
      <div 
        ...
        className="bg-white dark:bg-[#18181b] w-full max-w-2xl rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col max-h-[90vh] overflow-hidden animate-modal-pop"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex justify-between items-center px-6 py-4 border-b border-zinc-100 dark:border-zinc-800/80">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider bg-zinc-100 dark:bg-zinc-800/60 px-2 py-0.5 rounded border border-zinc-300 dark:border-zinc-700/60">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="mb-6">
              <label className="text-xs font-mono font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider mb-2 block">Description</label>
              <textarea
                value={description}
          ...
          <div className="w-full md:w-56 flex flex-col gap-6 shrink-0">
            ...
            {/* Color-coded labels */}
            <div>
              <label className="text-xs font-mono font-bold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider mb-3 block">Labels</label>
                ...
                {PREDEFINED_LABELS.map(label => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-0.5 text-[10px] font-mono font-bold tracking-wide uppercase rounded-md transition-all border ${
                        isActive
